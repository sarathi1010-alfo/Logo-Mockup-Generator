...
export async function generateStaticParams() {
  return programmaticPages
    .filter(page => page.type === 'mockup')
    .map(page => ({ slug: page.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params;
  const page = getProgrammaticPage(slug, 'mockup');
  ...
  return {
    ...
    alternates: {
      canonical: `${baseUrl}/mockups/${page.slug}`,
    }
  };
}

export default async function MockupCategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const page = getProgrammaticPage(slug, 'mockup');
